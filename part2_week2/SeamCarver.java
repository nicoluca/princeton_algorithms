...
public class SeamCarver {
    ...
    private double[][] calculateEnergyMatrix() {
        ...
        for (int column = 0; column < width(); column++)
            for (int row = 0; row < height(); row++)
                energyMatrix[column][row] = energy(column, row);
    ...
    public void removeVerticalSeam(int[] seam) {
        System.out.println("Removing vertical seam: " + Arrays.toString(seam));
    ...
    public static void main(String[] args) {
        ...
        if (!Paths.get(pathToPicture).toFile().exists())
            throw new IllegalStateException("File does not exist: " + pathToPicture);
